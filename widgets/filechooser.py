...
from local_config import SOURCE_PATH
...
Builder.load_file("./widgets/filechooser.kv")
...
class FileSystemLocal(FileSystemAbstract):
    ...
    def get_icon(self, fn):
        if self._remote_files is None:
            return 'none'
        fn_rel = str(Path(fn).relative_to(SOURCE_PATH))
        if self.is_dir(fn):
            is_synced = True
            for file_ in self._remote_files:
                if fn_rel in file_:
                    is_synced = False
                    break
        else:
            is_synced = fn_rel not in self._remote_files
        return 'success' if is_synced else 'error'
        ...
        if not is_root and not have_parent:
            ...
            if platform == 'win':
                ...
                pardir = self._create_entry_widget(dict(
                    ...
                    get_nice_size=lambda: '', icon= 'none'))
            else:
                pardir = self._create_entry_widget(dict(
    ...
    def _add_files(self, path, parent=None):
        ...
        for index, fn in enumerate(files):
            ...
            ctx = {'name': basename(fn),
                   ...
                   'parent': parent,
                   'sep': sep,
                   'icon': self.file_system.get_icon(fn)}
